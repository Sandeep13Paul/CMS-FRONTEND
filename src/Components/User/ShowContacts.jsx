...
const ShowContacts = () => {
  ...
  const fetchContacts = async () => {
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BACKEND_URI}/user/contacts/show?${query}`,
        {},
        {
          headers: {
      ...
      if (response.status === 200) {
        ...
        setContacts(data.content); // Extract contacts from the response

        // console.log(data);

        // Optionally, store page details for pagination controls
        setPageDetails({
  ...
  const handleSearch = () => {
    // const data = document.querySelector("#table-search-users");
    ...
    const handleDelete = (contact) => {
      try {
        const deleteContact = async () => {
          ...
          // console.log(query);
          const response = await axios.delete(`${import.meta.env.VITE_BACKEND_URI}/user/contacts/delete?${query}`,
            {
              headers: {
                Authorization: `Bearer ${token}`,
              },
        ...
        <div className="contacts_container p-8">
          <div className="relative overflow-x-auto shadow-md sm:rounded-lg">
            ...
            <div className="p-5 flex items-center justify-between flex-column flex-wrap md:flex-row space-y-4 md:space-y-0 pb-4 bg-gray-100 dark:bg-gray-900">
              ...
              <button type="button" onClick={handleSearch} className="py-2.5 px-5 me-2 mb-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Search</button>
            </div>
